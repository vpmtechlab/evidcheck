...
export function ChooseService({ onSelectService, selectedSlug }: ChooseServiceProps) {
  ...
  const resetServices = useMutation(api.init.resetAndSeedServices);

  // Auto-reset database if old AML records are present in Convex db
  useEffect(() => {
    if (services && services.some((s) => s.slug === "aml" || s.name === "AML")) {
  ...
  const displayServices = filteredFromDb.length > 0 ? filteredFromDb : [
    {
      _id: "seed_1",
